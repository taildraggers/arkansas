...
Daily aggregator of Arkansas-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "AR".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, AR` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Arkansas"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/arkansas/`
...
```html
<iframe
  src="https://taildraggers.github.io/arkansas/"
  title="Taildragger Ads in Arkansas"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_ARKANSAS_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
