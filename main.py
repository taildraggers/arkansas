"""Aggregates Arkansas-located listings from every taildraggers.com
...
ones whose location ends in ", AR".
...
This repo is a straight copy of the companion Alabama/Alaska/Arizona repos'
approach, with the state filter swapped to "AR".
"""
...
PAGE_TITLE = "Taildragger Ads in Arkansas"
STATE_ABBR = "AR"
...
_ARKANSAS_LOCATION_RE = re.compile(r",\s*AR\s*$", re.IGNORECASE)


def _is_arkansas(location: str) -> bool:
    return bool(_ARKANSAS_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        arkansas_rows = [row for row in rows if _is_arkansas(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(arkansas_rows)} in Arkansas")
        all_listings.extend(arkansas_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Arkansas listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Arkansas listings: {len(listings)}")
